...
async function buildWorkbook(rows, baseUrl) {
  ...
  for (const row of rows) {
    ...
    // Belge (PDF) varsa, dosya adi yerine tiklanabilir bir baglanti goster.
    if (row.foto_dosya && baseUrl) {
      const cell = addedRow.getCell('foto_dosya');
      cell.value = { text: 'Belgeyi Görüntüle (PDF)', hyperlink: `${baseUrl}/api/receipts/photos/${row.foto_dosya}` };
